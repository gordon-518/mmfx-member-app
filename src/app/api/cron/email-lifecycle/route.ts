...
import { renderLifecycle, templateFor, wrapLifecycle } from "@/lib/email/lifecycle";
...
export function contextFor(row: ClaimedSend, appUrl = APP_URL): LifecycleCtx | null {
  ...
  return {
    ...
    todayAnalysis:
      row.analysis_title && row.analysis_bias && BIASES.has(row.analysis_bias)
        ? {
            title: row.analysis_title,
            bias: row.analysis_bias as "bullish" | "bearish" | "neutral",
...
async function run(req: NextRequest) {
  ...
  if (spotlightIds.length) {
    const { data: rows } = await db
      ...
      .select("id, subject, html, text")
      .in("id", spotlightIds);
    for (const r of (rows ?? []) as (Spotlight & { id: string })[]) {
      spotlights.set(r.id, { subject: r.subject, html: r.html, text: r.text });
    }
  }
  ...
  for (const row of claimed) {
    ...
    try {
      ...
      if (!ctx) {
        ...
        // The brain composes the spotlight's BODY (design doc §5's bodyHtml /
        // bodyText); the rail frames it like any other step, so it carries the
        // same shell, risk line and unsubscribe link.
        const s = row.spotlight_id ? spotlights.get(row.spotlight_id) : undefined;
        if (!s) why = "approved spotlight row not found";
        else mail = wrapLifecycle({ subject: s.subject, html: s.html, text: s.text }, ctx);
  ...
  console.log(`[cron/email-lifecycle] claimed ${claimed.length}, sent ${sent}, failed ${failed}`);
