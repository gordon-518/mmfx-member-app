...
// Checklist steps a browser may mark (the rest are read from where they happen).
const CLIENT_STEPS = ["lesson-1"] as const;

export interface ClientEventProps {
  broker?: (typeof BROKERS)[number];
  flow?: (typeof FLOWS)[number];
  channel?: (typeof CHANNELS)[number];
  step?: (typeof CLIENT_STEPS)[number];
}
...
export function sanitizeClientProps(event: ClientEvent, raw: unknown): ClientEventProps {
  ...
  if (event === "onboarding_step_done") {
    const step = pick(r.step, CLIENT_STEPS);
    return step ? { step } : {};
  }
