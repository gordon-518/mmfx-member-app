...
| `trial_ends_at` | `timestamptz` | — | yes | Set at signup to `signup_at + 14 days` (signups from 15 Jul to 10 Sep 2026 got 7 days; restored to 14 in `20260910000003_trial_14_days.sql`, with no backfill). **A `member_active` row may keep it** (`20260915000004_trial_fairness.sql`): a trial user who deposits while their clock is still running keeps it, and the app's `accessTierFor` gives them the trial's full access until it passes, then the tier their deposits earned. An expired or absent clock is still cleared on verify. Stored, never computed on the fly. **Null for members** — cleared on the flip to `member_active` (2026-06-10). A CHECK guarantees `trial_active`/`re_trial_active` always have a clock. |
...
| `lifetime_plan` | `text` | — | yes | US/UK lifetime plan (conversion-fix Phase 6): `team` (Team MM Access, USD 588: Team MM tier without the full course) or `team_mentorship` (USD 1,588: everything). Set only by `fn_admin_grant_lifetime(target_user_id, p_plan)` (admin-only, `20260915000003`), which makes the account `member_active`, clears the trial clock, and never downgrades Mentorship to Team only. Lifetime members need no `trading_account_number`, and without one the AI Trading Assistant has nothing to connect. |
| `lifetime_granted_at` | `timestamptz` | — | yes | First grant of the lifetime plan; an upgrade keeps it. |
...
**Onboarding for SendPulse (4.3):** `fn_onboarding_states()` (**service_role only**, `20260915000002`) returns `(user_id, steps, done)` for every profile, reading the same five steps as `fn_my_onboarding`. The nightly `sendpulseSync` stamps them on each contact as `onboarding` (e.g. `tv,analysis` or `none`) and `onboarding_done` (0–5).
