...
- [x] **3.2 Let `fn_verify_deposit` handle tiers and top-ups.**
  ...
  **Landed (10 Sept, `20260910000006_verify_deposit_tiers.sql`, applied to prod), shipped after 3.3 so a $50 member is Foundation, not full access:**
  ...
  - **Review fix:** a double-submit guard in `fn_verify_deposit` refuses the same amount and broker for the same user within 2 minutes. Dropping the "already a member" rejection had left a double-clicked Verify able to record one deposit twice. Proved on prod: a repeat $150 is refused. `verifyDeposit` logs a failed pre-read, since Purchase is then skipped.
...
- [x] **5.1b Telegram username required (15 Sept, Gordon).** The deposit form requires the member's Telegram @handle (`deposit_submissions.telegram_username`; `fn_submit_deposit` gains `p_telegram_username`, `20260915000005`). It appears in the admin queue as a t.me link and in the admin's Telegram DM.
- [x] **5.2 Build an admin review queue.** Pending submissions with the proof image. **Verify** calls the Phase 3 RPC; **Reject** takes a reason that's shown to the user.
  **Landed (`20260910000009_deposit_review.sql`, applied to prod):** `/admin` opens with the queue (`DepositQueue`): pending submissions oldest first, each showing the member's email, tier, amount, first deposit or top-up, broker, account and TradingView handle, plus a one-hour signed link to the proof. **Verify** (with an IB-confirmed checkbox) and **Reject** (reason required) both go through `fn_review_deposit_submission`. It locks the row and requires `pending`, so a submission can't be reviewed twice. Verify runs `fn_verify_deposit`, so every Phase 3 rule applies, and saves the account number when the profile has none. The manual verify form and the queue now share one `afterVerify` path: TradingView sync, `deposit_verified` and `tier_changed` events, and CAPI Purchase on a first deposit only.
  **Verified:** a 10-check rolled-back scenario on prod. It covered a non-admin refused, a $100 verify producing a member plus ledger row plus saved account number, a second review refused, missing IB confirmation refused, and reject requiring a reason. In the browser, only the queue's rendering is checked. I didn't click Verify on a test user, because a first-deposit verify sends a real CAPI Purchase to Meta.

- [x] **5.3 Add notifications.** A Telegram DM to the admin on each new submission (reuse `src/lib/telegram.ts` / `notify.ts`), and an email to the user on verify or reject.
  **Landed:** `submitDeposit` sends the admin Telegram DM ("New deposit submission: email, $amount, broker, link to /admin") through `sendTelegram`; it's best-effort, and a failure doesn't fail the submission. `reviewSubmission` emails the member from hello@ using `src/lib/depositEmails.ts`: "Verified: you're on <tier>" with the amount and running total, or the rejection reason with a link back to the form. Both carry the risk line and are compliance-tested. The live sends weren't triggered in testing, to avoid DMing the real admin chat and emailing test addresses.
